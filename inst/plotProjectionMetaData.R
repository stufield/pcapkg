...
#' @examples
#' apts <- SomaDataIO::getAnalytes(sim_test_data)
#' plotProjectionMetaData(log10(sim_test_data), aptamers = apts)
...
plotProjectionMetaData <- function(data, dims = 1:2L, ...) {
  ...
  meta <- dplyr::select_if(data[, SomaDataIO::getMeta(data)], good_meta) |>
